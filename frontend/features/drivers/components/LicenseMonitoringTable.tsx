...
interface LicenseMonitoringTableProps {
  records: LicenseMonitoringRecord[];
  totalItems: number;
  totalPages: number;
  page: number;
...
export const LicenseMonitoringTable = memo(function LicenseMonitoringTable({
  records,
  totalItems,
  totalPages,
  page,
        ...
        <table className="w-full text-left border-collapse min-w-[1000px]">
          <thead>
            <tr className="bg-gray-50 border-b border-border-app">
              ...
              <th onClick={() => onSort("daysRemaining")} className={headerClass}>
                <div className="flex items-center gap-1.5">
          ...
          <tbody className="divide-y divide-gray-100 text-sm">
            {records.map((rec) => (
              <tr key={rec.id} className="hover:bg-primary-light/20 transition-colors">
                ...
                <td className="p-4 font-mono font-semibold text-xs text-text-secondary">
                  {rec.licenseNumber}
                </td>
                <td className="p-4 text-xs font-mono text-text-secondary">
                  {rec.licenseExpiry}
                  <span className="block mt-0.5 select-none">
                    <ExpiryStatusBadge status={rec.licenseStatus} />
                  </span>
                </td>
                <td className="p-4 text-xs font-mono text-text-secondary">
                  {rec.medicalExpiry}
                  <span className="block mt-0.5 select-none">
                    <ExpiryStatusBadge status={rec.medicalStatus} />
                  </span>
                </td>
                <td className="p-4 font-mono text-xs text-right pr-12">
                  <span className={`font-bold text-sm ${
                    ...
                    {rec.daysRemaining <= 0 ? 0 : rec.daysRemaining}
                  </span>
                  <span className="text-[10px] text-text-muted font-sans font-normal ml-0.5">days</span>
                </td>
                <td className="p-4 text-center">
                  <ExpiryStatusBadge status={rec.overallStatus} />
                </td>
                <td className="p-4">
                  <DocumentActions driverId={rec.id} />
                </td>
              </tr>
            ))}
          </tbody>
